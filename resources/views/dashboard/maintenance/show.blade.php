    ...
    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-xl">
        ...
        <div class="mt-6 grid gap-6 lg:grid-cols-2">
                    ...
                    <ol class="mt-3 space-y-3">
                        @forelse($asset->maintenances as $maintenance)
                            <li class="rounded-xl border border-slate-200 p-3">
                                <p class="text-xs font-semibold text-slate-500">{{ optional($maintenance->performed_at)->format('d-m-Y H:i') }}</p>
            ...
            <form method="POST" action="{{ route('dashboard.maintenance.store', ['asset' => $asset, 'role' => $role]) }}" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <div><label class="mb-2 block text-sm font-semibold text-slate-700">Fecha y hora</label><input name="performed_at" type="datetime-local" value="{{ old('performed_at', now()->format('Y-m-d\TH:i')) }}" required class="w-full rounded-2xl border border-slate-300 px-4 py-3"></div>
